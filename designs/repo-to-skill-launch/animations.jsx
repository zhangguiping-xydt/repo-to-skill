...
function detectRecordMode() {
  try {
    const params = new URLSearchParams(window.location.search);
    if (!params.has('record')) return false;
...
function Stage({
  ...
  const recordMode = detectRecordMode();

  const [time, setTime] = React.useState(() => {
    if (captureMode || recordMode) return 0; // deterministic: ignore persisted playhead
    try {
  ...
  React.useEffect(() => {
    if (captureMode || recordMode) return;
    try { localStorage.setItem(persistKey + ':t', String(time)); } catch {}
  }, [time, persistKey, captureMode, recordMode]);

  // Auto-scale to fit viewport (forced to 1:1 in capture mode)
  React.useEffect(() => {
    if (captureMode || recordMode) { setScale(1); return; }
  ...
  }, [width, height, captureMode, recordMode]);
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        width: '100%',
        display: 'flex',
        alignItems: (captureMode || recordMode) ? 'flex-start' : 'center',
        justifyContent: (captureMode || recordMode) ? 'flex-start' : 'center',
        overflow: (captureMode || recordMode) ? 'visible' : 'hidden',
        minHeight: 0,
      }}>
        <div
          ref={canvasRef}
          ...
          style={{
            ...
            background,
            position: 'relative',
            transform: (captureMode || recordMode) ? 'none' : `scale(${scale})`,
            transformOrigin: 'center',
            flexShrink: 0,
            boxShadow: (captureMode || recordMode) ? 'none' : '0 20px 60px rgba(0,0,0,0.4)',
            overflow: 'hidden',
          }}
        >
